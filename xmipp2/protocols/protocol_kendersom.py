...
class KendersomBaseClassify(ProtClassify2D):
    ...
    def _defineParams(self, form):
        ...
        form.addParam('inputParticles', PointerParam, label="Input images", important=True,
                      pointerClass='SetOfImages', help='Select the input images from the project.')
    ...
    def _methods(self):
        ...
        if not hasattr(self, 'outputClasses'):
            ...
            messages.append('%s particles from %s were classified to obtain %s classes %s.'
                            % (self.inputParticles.get().getSize(), self.getObjectTag('inputParticles'),
                               self.outputClasses.getSize(), self.getObjectTag('outputClasses')))
            if self.useMask:
